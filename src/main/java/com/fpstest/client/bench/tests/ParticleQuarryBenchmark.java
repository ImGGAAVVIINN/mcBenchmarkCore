...
public final class ParticleQuarryBenchmark implements Benchmark {
   ...
   private static final ParticleEffect[] EMITTERS = new ParticleEffect[]{
      ParticleTypes.FLAME,
      ParticleTypes.SMOKE,
      ParticleTypes.CLOUD,
      ...
      ctx.onServer(s -> {
         ...
         if (lvl != null) {
            ...
            for (int e = 0; e < EMITTERS.length; e++) {
               ParticleEffect type = EMITTERS[e];
               ...
               for (int i = 0; i < 40; i++) {
                  ...
                  double pz = ez + jz;
                  if (players.isEmpty()) {
                     lvl.spawnParticles(type, px, py, pz, 1, 0.0, 0.0, 0.0, 0.04);
                  } else {
                     for (ServerPlayerEntity p : players) {
                        lvl.spawnParticles(p, type, true, false, px, py, pz, 1, 0.0, 0.0, 0.0, 0.04);
                     }
                  }
